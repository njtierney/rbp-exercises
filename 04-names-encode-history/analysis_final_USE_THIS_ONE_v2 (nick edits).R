...
dat$bill_ratio <- dat$bill_length_mm / dat$bill_depth_mm

# other crew measured the 2007 birds, drop them
dat <- dat[dat$year >= 2008, ]

# one bird recorded at 6.3kg, has to be a typo
dat <- dat[dat$mass_kg < 6, ]

fit <- lm(mass_kg ~ flipper_length_mm * species + sex, data = dat)
summary(fit)
